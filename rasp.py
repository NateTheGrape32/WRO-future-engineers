...
ROI_LEFT = [0, 265, 240, 385]
ROI_RIGHT = [400, 265, 640, 385]

LAB_LOWER = np.array([0, 0, 0], dtype=np.uint8)
LAB_UPPER = np.array([70, 255, 255], dtype=np.uint8)

ENTER_TURN_THRESH = 4000
EXIT_TURN_THRESH = 8800
...
def find_wall_area(frame, roi):
    ...
    lab = cv2.GaussianBlur(lab, (9, 9), 0)
...
def pd_controller(error, kp=0.2, kd=0.1):
    global prevError
